...
function renderCurrentGamedex() {
    ...
    detailContent.innerHTML = `
        <article class="gamedex-card type-${primaryType}">
            <div class="gamedex-media-tools" aria-label="Controles multimedia">
                <button class="gamedex-media-button" type="button" data-gamedex-animation aria-pressed="${currentImageMode === 'animated'}" title="${currentImageMode === 'animated' ? 'Mostrar imagen estática' : 'Mostrar sprite animado'}">
                    ${currentImageMode === 'animated' ? 'GIF' : 'IMG'}
                </button>
                ${shinyAvailable ? `<button class="gamedex-media-button" type="button" data-gamedex-shiny aria-pressed="${currentShiny}" title="Cambiar entre normal y shiny">${currentShiny ? 'SHINY' : 'SHINY'}</button>` : ''}
                <button class="gamedex-media-button" type="button" data-gamedex-audio aria-pressed="${getStoredBoolean(AUDIO_PREFERENCE_KEY, true)}" title="${getStoredBoolean(AUDIO_PREFERENCE_KEY, true) ? 'Desactivar sonidos' : 'Activar sonidos'}">${getStoredBoolean(AUDIO_PREFERENCE_KEY, true) ? '♫' : '♪'}</button>
            </div>
            ...
            <div class="gamedex-artwork" aria-label="Ilustración de ${escapeAttribute(displayPokemonName(pokemon))}">
                ...
                <img class="${currentShiny ? 'is-shiny' : ''} ${currentImageMode === 'animated' && animatedAvailable ? 'is-animated' : ''}"
        ...
        <section class="pokemon-about">
            <div class="pokemon-about-header"><span>Pokédex</span><h2>Sobre ${escapeHtml(displayPokemonName(pokemon))}</h2></div>
...
function createFormSelector(species, pokemon) {
    ...
    return `
        <div class="gamedex-form-controls">
            ...
            <select class="gamedex-form-select" id="gamedex-form-select" data-gamedex-form>${options}</select>
        </div>
    `;
}
...
function createSpeciesSection(species) {
    ...
    return `
        <section class="gamedex-species">
            <div class="gamedex-species-header"><span>Ficha de especie</span><h2>Información de especie</h2></div>
            <div class="gamedex-species-grid">
                ${createSpeciesItem('Hábitat', habitat)}
                ${createSpeciesItem('Generación', generation)}
                ${createSpeciesItem('Especie', species.genera?.find(({ language }) => language.name === 'es')?.genus ?? 'Pokémon')}
            </div>
            ${varieties.length > 1 ? `<div class="gamedex-forms"><span class="gamedex-forms-title">Formas disponibles</span><div class="gamedex-forms-list">${varieties.map(({ pokemon: variant }) => `<button class="gamedex-form-chip ${variant.name === currentPokemon.name ? 'is-current' : ''}" type="button" data-gamedex-form-chip="${escapeAttribute(variant.name)}">${escapeHtml(formatVariantName(variant.name, variant.name === species.name))}</button>`).join('')}</div></div>` : ''}
        </section>
    `;
}
...
function createStat(stat) {
    const labels = { hp: 'PS', attack: 'ATQ', defense: 'DEF', 'special-attack': 'ATQ ESP', 'special-defense': 'DEF ESP', speed: 'VEL' };
    ...
    return `<div class="gamedex-stat"><div class="gamedex-stat-top"><span>${escapeHtml(label)}</span><strong>${value}</strong></div><div class="gamedex-stat-track"><span style="width:${width}%"></span></div></div>`;
}
...
function createEvolutionSection(tree) {
    ...
    if (tree.children.length === 1) {
        return `<section class="evolution-section"><span class="panel-eyebrow">Evoluciones</span><h2>Cadena evolutiva</h2><div class="evolution-chain evolution-chain-linear">${rootCard}<span class="evolution-arrow" aria-hidden="true"></span>${createLinearDescendant(tree.children[0])}</div></section>`;
    }

    return `<section class="evolution-section evolution-section-branching"><span class="panel-eyebrow">Evoluciones</span><h2>Cadena evolutiva</h2><div class="evolution-tree"><div class="evolution-tree-root">${rootCard}</div><div class="evolution-tree-connector" aria-hidden="true"></div><div class="evolution-tree-branches">${tree.children.map((child) => `<div class="evolution-branch"><span class="evolution-arrow" aria-hidden="true"></span>${createLinearDescendant(child)}</div>`).join('')}</div></div></section>`;
}
...
function createLinearDescendant(node) {
    ...
    if (current.children?.length > 1) {
        cards.push(`<div class="evolution-subbranches">${current.children.map((child) => `<div class="evolution-branch"><span class="evolution-arrow" aria-hidden="true"></span>${createLinearDescendant(child)}</div>`).join('')}</div>`);
    }
...
function createEvolutionCard(pokemon, details = []) {
    ...
    return `<article class="evolution-card" data-pokemon-id="${pokemon.id}"><button class="evolution-card-button" type="button" data-pokemon-id="${pokemon.id}" aria-label="Ver ${escapeHtml(capitalize(pokemon.name))}"><span class="evolution-number">#${formatId(pokemon.id)}</span><img src="${escapeAttribute(getEvolutionImage(pokemon.id))}" alt="${escapeAttribute(capitalize(pokemon.name))}" loading="lazy"><strong>${escapeHtml(capitalize(pokemon.name))}</strong>${condition ? `<small class="evolution-card-condition">${escapeHtml(condition)}</small>` : ''}</button></article>`;
}
...
function getStaticImage(pokemon, shiny = false) {
    ...
    return (shiny ? artwork?.front_shiny : artwork?.front_default) || pokemon.sprites?.[shiny ? 'front_shiny' : 'front_default'] || '';
}
...
function getAnimatedImage(pokemon, shiny = false) {
    ...
    return (shiny ? showdown?.front_shiny : showdown?.front_default) || '';
}
...
function getDescription(species) {
    const entry = species.flavor_text_entries?.find(({ language }) => language.name === 'es') || species.flavor_text_entries?.find(({ language }) => language.name === 'en');
    if (!entry) return 'No hay descripción disponible.';
    return entry.flavor_text.replace(/\f/g, ' ').replace(/\n/g, ' ');
}

function formatHeight(height) { return `${(height / 10).toFixed(1)} m`; }
function formatWeight(weight) { return `${(weight / 10).toFixed(1)} kg`; }
...
function displayPokemonName(pokemon) { return pokemon.form_name ? `${pokemon.species?.name ?? pokemon.name} · ${pokemon.form_name}` : capitalize(pokemon.name); }
function formatItem(value) { return capitalize(String(value).replaceAll('-', ' ')); }
function formatVariantName(name, isDefault = false) { if (isDefault) return 'Normal'; return capitalize(String(name).replaceAll('-', ' ')); }
function formatGeneration(value) { const match = String(value).match(/generation-([ivx]+)/i); return match ? `Generación ${match[1].toUpperCase()}` : capitalize(String(value).replaceAll('-', ' ')); }
function translateHabitat(value) { const translations = { cave: 'Cueva', forest: 'Bosque', grassland: 'Pradera', mountain: 'Montaña', rare: 'Raro', roughTerrain: 'Terreno accidentado', sea: 'Mar', urban: 'Urbano', watersEdge: 'Orilla del agua' }; return translations[value] ?? formatItem(value); }
function translateEvolutionTrigger(value) { const translations = { level-up: 'Sube de nivel', trade: 'Intercambio', use-item: 'Usar objeto', shed: 'Muda', spin: 'Girar', tower-of-darkness: 'Torre de la Oscuridad', tower-of-waters: 'Torre de las Aguas', three-critical-hits: 'Tres golpes críticos', take-damage: 'Recibir daño' }; return translations[value] ?? formatItem(value); }
function translateType(type) { const translations = { normal: 'Normal', fire: 'Fuego', water: 'Agua', electric: 'Eléctrico', grass: 'Planta', ice: 'Hielo', fighting: 'Lucha', poison: 'Veneno', ground: 'Tierra', flying: 'Volador', psychic: 'Psíquico', bug: 'Bicho', rock: 'Roca', ghost: 'Fantasma', dragon: 'Dragón', dark: 'Siniestro', steel: 'Acero', fairy: 'Hada' }; return translations[type] ?? capitalize(type); }
function translateAbility(ability) { const translations = { overgrow: 'Espesura', blaze: 'Mar Llamas', torrent: 'Torrente', shield_dust: 'Polvo Escudo', static: 'Electricidad Estática' }; return translations[ability] ?? formatItem(ability); }

function getStoredBoolean(key, fallback) { try { const value = localStorage.getItem(key); return value === null ? fallback : value === 'true'; } catch { return fallback; } }
function setStoredBoolean(key, value) { try { localStorage.setItem(key, String(value)); } catch { /* storage unavailable */ } }
function scheduleCry(pokemon) { window.setTimeout(() => playCry(pokemon), 180); }
function playCry(pokemon) { const url = pokemon.cries?.latest || pokemon.cries?.legacy; if (!url || !getStoredBoolean(AUDIO_PREFERENCE_KEY, true)) return; const audio = new Audio(url); audio.volume = 0.35; audio.play().catch(() => {}); }
...
detailContent?.addEventListener('click', (event) => {
    ...
    if (formChip) { event.preventDefault(); selectForm(formChip.dataset.gamedexFormChip); return; }

    const animationButton = event.target.closest('[data-gamedex-animation]');
    if (animationButton) {
        const next = currentImageMode !== 'animated';
